...
def prep_telco(df):
    ...
    telco_new["paperless_billing_encoded"] = telco.paperless_billing.map(
        {"Yes": 1, "No": 0}
    )
    telco_new["churn_encoded"] = telco.churn.map({"Yes": 1, "No": 0})
    telco_new["total_in_household"] = (
        telco_new.dependents_encoded + telco_new.partner_encoded + 1
    )
    telco = telco.drop(
        columns=[
            "gender",
...
def split_data(df, tostratify=None, test_size=0.2, validate_size=0.25):
    """
    Takes in a dataframe and return train, validate, test subset dataframes
    """
    if tostratify != None:
        train, test = train_test_split(
            df, test_size=test_size, random_state=123, stratify=df[tostratify]
        )
        train, validate = train_test_split(
            train, test_size=validate_size, random_state=123, stratify=train[tostratify]
        )
    else:
        train, test = train_test_split(df, test_size=test_size, random_state=123)
        train, validate = train_test_split(
            train, test_size=validate_size, random_state=123
        )
    ...
    df = pd.DataFrame(
        ...
        index=["Prepared Data", "Train", "Validate", "Test"],
...
def imput_telco(df):
    ...
    df.total_charges = df.total_charges[df.total_charges.str.strip() != ""].astype(
        float
    )
...
def num_to_num(df, num_combos, alpha=0.05):
...
def cat_to_cat(df, cat_combos, alpha=(100 - 95) / 100, rejected_chi=True):
    ...
    if rejected_chi == True:
        ...
        reject.rename(
            columns={0: "Chi Square Stat", 1: "Chi Square p", 2: "Vars"}, inplace=True
        )
        display(
            symbols("\chi^2"),
            symbols("We~reject~the~null~hypothesis"),
            symbols("Thus~dependendace~is~likely"),
        )
        display(reject)
        return reject
        ...
        display(
            symbols("\chi^2"),
            symbols("We~accept~the~null~hypothesis"),
            symbols("Thus~independence~is~likely"),
        )
        display(accept)
        return accept
    else:
...
def cat_to_num(df, cat_numcombos, alpha=0.05, rejected_null=True):
    ...
    if rejected_null == True:
        reject = pd.DataFrame(reject)
        reject.rename(
            columns={0: "Man Whit Stat", 1: "Alt p p", 2: "Vars"}, inplace=True
        )
        display(
            symbols("Mann~Whitney"),
            symbols("We~reject~the~null~hypothesis"),
            symbols("Hence~it~is~unlikely~they~are~from~same~population"),
            reject,
        )
        return reject
        ...
        display(
            symbols("Mann~Whitney"),
            symbols("We~accept~the~null~hypothesis"),
            symbols("Hence~it~is~likely~they~are~from~same~population"),
            accept,
        )
        return accept
