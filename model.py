...
def decision_tree(X_train, y_train, X_validate, y_validate, X_test, y_test, random):
    ...
    grid_search_tree = GridSearchCV(
        estimator=tree_model, param_grid=tree_params, cv=20, scoring="recall", n_jobs=-2
    )
    ...
    tree_opt_dict[f"Parameters:{gs_tree_params}]"] = {
        ...
        "diff": round(
            abs(
                tree_opt.score(X_train, y_train)
                - tree_opt.score(X_validate, y_validate)
            ),
            2,
        ),
    }
...
def random_forest(X_train, y_train, X_validate, y_validate, X_test, y_test, random):
    ...
    grid_search_rf = GridSearchCV(
        estimator=forest1, param_grid=forest_params, cv=20, scoring="recall", n_jobs=-2
    )
    ...
    opt_rf_dict[f"Parameters:{gs_rf_params}]"] = {
        ...
        "diff": round(
            abs(opt_rf.score(X_train, y_train) - opt_rf.score(X_validate, y_validate)),
            2,
        ),
    }
    ...
    disp = ConfusionMatrixDisplay(
        confusion_matrix=rf_cm, display_labels=opt_rf.classes_
    )
    disp.plot()
    plt.show()
...
def logistic_reg(X_train, y_train, X_validate, y_validate, X_test, y_test, random):
    ...
    grid_search_log = GridSearchCV(
        estimator=logit, param_grid=logit_params, cv=20, scoring="recall", n_jobs=-2
    )
    ...
    log_opt_dict[f"Parameters:{gs_log_params}]"] = {
        ...
        "diff": round(
            abs(
                log_opt.score(X_train, y_train) - log_opt.score(X_validate, y_validate)
            ),
            2,
        ),
    }
...
def knn(X_train, y_train, X_validate, y_validate, X_test, y_test):
    ...
    grid_search_knn = GridSearchCV(
        estimator=knn, param_grid=knn_params, cv=20, scoring="recall", n_jobs=-2
    )
    ...
    knn_opt_dict[f"Parameters:{gs_knn_params}]"] = {
        ...
        "diff": round(
            abs(
                knn_opt.score(X_train, y_train) - knn_opt.score(X_validate, y_validate)
            ),
            2,
        ),
    }
...
def confusion_matrix_analyis(tn, fp, fn, tp):
    ...
    dfindex = ["tn", "fp", "fn", "tp", "npv", "ppv", "acc", "recall", "mcc", "f1"]

    dfvals = [tn, fp, fn, tp, npv, ppv, acc, recall, mcc, f1]
...
def best_model_recall(X_train, y_train, X_validate, y_validate, X_test, y_test, random):
    ...
    tntst, fptst, fntst, tptst = confusion_matrix(
        y_test, y_pred_forest_opt_test
    ).ravel()
    ...
    forest_report = classification_report(
        y_test, y_pred_forest_opt_test, output_dict=True
    )
    display(pd.DataFrame(forest_report))
    # sklearn confusion matrix
...
def best_model_precision(
    X_train, y_train, X_validate, y_validate, X_test, y_test, random
):
    # Make the model
    forest1 = RandomForestClassifier()
